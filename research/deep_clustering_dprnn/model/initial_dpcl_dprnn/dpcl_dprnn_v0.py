    ...
    def forward(self, x, is_train=True):
        '''
           ...
           return: 
                  ...
                  for test: TF x D
        '''
        print("Mode: is_train in dpcl_dprnn ", is_train)
        # print("x.shape before not is_train ", x.data.size())
        # To fix test.py, this part of the code was moved in the dprnn block because its where B is manipulated
        # It takes in a 2dim tensor [?, NFFT] when training
        # if not is_train:
        #     x = torch.unsqueeze(x, 0) # this forces a B = 1 into a [T, F] tensor to [1, T, F]
        ...
        # print("x.shape after self.dprnn ", x.data.size())
...
class Dual_Path_RNN(nn.Module):  # The DPRNN block all together # Has conv tasnet layers
    ...
    def forward(self, x, is_train=True):
        ...
        print("Mode: is_train in self.dprnn ", is_train)
        print("x.shape before norm: ", x.data.shape)
        x = self.norm(x)

        # transform into [B, T(L), F(N)]
        if is_train:
            x, _ = pad_packed_sequence(x, batch_first=True)
            print("pad_packed_sequence triggered", x.data.size())
        else:
            x = torch.unsqueeze(x, 0)
            print("torch unsqueeze triggered", x.shape)
        ...
        if is_train:
            x = pack_sequence(x)
            print("pack_sequence(x) triggered", x.data.size())
