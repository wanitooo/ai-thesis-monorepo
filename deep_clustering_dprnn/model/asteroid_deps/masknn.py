...
class DPRNNBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.reshape(B, K, L, N).transpose(
            1, -1).transpose(2, -1).contiguous()
        x = self.inter_norm(x)
        return output + x
...
class DPRNN(nn.Module):
    ...
    def __init__(
        ...
        self.net_gate = nn.Sequential(
            nn.Conv1d(bn_chan, bn_chan, 1), nn.Sigmoid())
...
# class DCCRMaskNet(BaseDCUMaskNet):
    ...
    def __init__(self, encoders, decoders, n_freqs, **kwargs):
        ...
        last_encoder_out_shape = (
            encoders[-1][1], int(np.ceil(n_freqs / freq_prod)))
        ...
        super().__init__(
            encoders=[
                *(DCUNetComplexEncoderBlock(*args, activation="prelu")
                  for args in encoders),
                DCCRMaskNetRNN(np.prod(last_encoder_out_shape)),
            ],
            decoders=[
                torch.nn.Identity(),
                *(DCUNetComplexDecoderBlock(*args, activation="prelu")
