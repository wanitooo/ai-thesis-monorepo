...
def main(args):
    ...
    # Get train dir
    # subdir2 = os.path.join(wham_noise_dir, 'tr')
    subdir = os.path.join(os.path.abspath(wham_noise_dir), 'tr')
    # print("SUBDIR~~~~~~", subdir)
    # print("SUBDIR2~~~~~~", subdir2)
    # List files in that dir
    sound_paths = glob.glob(os.path.join(os.path.abspath(subdir), '**/*.wav'),
                            recursive=True)
    print("sound_paths~~~~~~", sound_paths)
    ...
    if len(sound_paths) == 60000:
        ...
        to_be_removed_08 = [x.replace('sp08', '')
                            for x in sound_paths if 'sp08' in x]
        to_be_removed_12 = [x.replace('sp12', '')
                            for x in sound_paths if 'sp12' in x]
